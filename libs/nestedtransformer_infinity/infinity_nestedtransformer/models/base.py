...
class NestedTransformerBaseLayer:
    def __init__(
        self,
        config: "PretrainedConfig",
    ):
        r"""
        Base layer for `NestedTransformer` integration. This class is used to wrap all the necessary
        components for the `NestedTransformer` integration.
    ...
    def validate_bettertransformer(self):
        r"""
        A wrapper function to validate the `NestedTransformer` implementation. Implements most relevant checks
        ...
        if self.num_heads is None:
            raise ValueError(
                "Number of heads not set for `NestedTransformer` integration."
            )

        if self.embed_dim is None:
            raise ValueError(
                "Embedding dimension not set for `NestedTransformer` integration."
            )

        if self.norm2_eps is None or self.norm1_eps is None:
            raise ValueError(
                "`norm2_eps` and `norm1_eps` not set for `NestedTransformer` integration."
            )

        # Check positional embedding
        if self.pos_emb_type is not None and self.pos_emb_type != "absolute":
            raise ValueError(
                f"Positional embedding type {self.pos_emb_type} not "
                "supported for `NestedTransformer` integration"
            )

        # Check norm1 epsilon and norm2 epsilon equality
        if self.norm1_eps != self.norm2_eps:
            raise ValueError(
                "norm1_eps and norm2_eps must be equal for `NestedTransformer` integration."
            )

        # Check activation function
        if self.act_fn in USE_AT_OWN_RISK_ACTIVATION_FUNCTIONS:
            logger.warning(
                f"Overriding {self.act_fn} activation with gelu. Use the transformed model at your own risk, the output logits could be significantly different."
            )
            self.act_fn = "gelu"
        elif self.act_fn not in SUPPORTED_ACTIVATION_FUNCTIONS:
            raise ValueError(
                f"Activation function {self.act_fn} not supported"
                " for `NestedTransformer` integration."
            )
        ...
        if self.module_mapping is not None:
            if "" in self.module_mapping.values():
                ...
                raise NotImplementedError(
                    "replacing a submodule in revert is not supported"
                )

        for (
            modified_layer_key_names,
            original_layer_key_names,
        ) in self.original_layers_mapping.items():
            if isinstance(original_layer_key_names, list):
                ...
                for i, subparam_name in enumerate(original_layer_key_names):
                    ...
                    if module not in self.keys_to_ignore:
                        ...
                        parameter = current_weight[
                            i * split_index : (i + 1) * split_index
                        ].clone()
                        if isinstance(
                            recurse_getattr(module, subparam_name), torch.nn.Parameter
                        ):
                ...
                if isinstance(
                    recurse_getattr(module, original_layer_key_names),
                    torch.nn.Parameter,
                ):
                    parameter = torch.nn.Parameter(parameter)
