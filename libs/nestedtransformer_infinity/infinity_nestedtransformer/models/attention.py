...
def raise_on_head_mask(head_mask: Optional[torch.Tensor]):
    if head_mask is not None:
        raise ValueError(
            "layer_head_mask (or head_mask) different than None is unsupported for now with NestedTransformer, please"
...
def gpt2_wrapped_scaled_dot_product(
    ...
    if (
        ...
        raise ValueError(
            "NestedTransformer does not support padding='max_length' with a batch size of 1."
        )
    ...
    def t5_forward(
        ...
        if past_key_value is not None:
            ...
            if is_cross_attention:
                # after the first generated id, we can subsequently reuse all key/value_states from cache
        ...
        if is_cross_attention and past_key_value is not None and is_updated:
            ...
            if past_key_value is not None:
                ...
                # set flag that curr layer for cross-attn is already updated so we can reuse in subsequent calls
                if is_cross_attention:
    ...
    def t5_forward(
        ...
        **kwargs,
        ...
        if len(self.pruned_heads) > 0:
            raise ValueError(
                f"Setting `pruned_heads` is unsupported with NestedTransformer, found {self.pruned_heads}."
            )
...
if check_if_transformers_greater("4.44"):
    ...
    def bloom_forward(
        ...
        if attention_mask is not None:  # no matter the length, we just slice it
            kv_length = (
                cache_position[-1] + 1
            )  # cache position is 0-indexed while length should start from 1
            causal_mask = attention_mask[:, :, :, :kv_length]
            alibi = torch.masked_fill(
                alibi, causal_mask.bool(), torch.finfo(alibi.dtype).min
            )
        ...
        **kwargs,
        ...
        if layer_past is not None:
            ...
            key_layer = key_layer.reshape(
                batch_size, self.num_heads, *key_layer.shape[1:]
            )
            value_layer = value_layer.reshape(
                batch_size, self.num_heads, *value_layer.shape[1:]
            )
        else:
        ...
        output_tensor = torch.nn.functional.dropout(
            output_tensor, p=self.hidden_dropout, training=self.training
        )
