...
from .base import NestedTransformerBaseLayer
...
class AlbertLayerNestedTransformer(NestedTransformerBaseLayer, nn.Module):
    def __init__(self, albert_layer, config):
        r"""
        A simple conversion of the ALBERT layer to its `NestedTransformer` implementation.
        ...
        super(NestedTransformerBaseLayer, self).__init__()
        # In_proj layer
    ...
    def forward(self, hidden_states, attention_mask, *_):
        if (
            ...
            and not torch.is_autocast_cpu_enabled()
        ):
            ...
            if attention_mask is not None:
                ...
                attention_mask = torch.reshape(
                    attention_mask, (attention_mask.shape[0], attention_mask.shape[-1])
                )
            ...
            qkv = F.linear(
                hidden_states, weight=self.in_proj_weight, bias=self.in_proj_bias
            )

            qkv = qkv.view(
                qkv.size()[:-1] + (3, self.num_heads, self.attention_head_size)
            ).permute(2, 0, 3, 1, 4)
            ...
            new_attention_out_shape = attention_out.size()[:-2] + (
                self.num_heads * self.attention_head_size,
            )
            ...
            # BertIntermediate
            hidden_states = self.act_fn_callable(
                F.linear(attention_out, self.linear1_weight, self.linear1_bias)
            )
...
class BertLayerNestedTransformer(NestedTransformerBaseLayer, nn.Module):
    def __init__(self, bert_layer, config):
        r"""
        A simple conversion of the BERT layer to its `NestedTransformer` implementation.
    ...
    def forward(self, hidden_states, attention_mask, *_):
        # No check on output_attentions here as roformer relies on BertLayerNestedTransformer but does not pass output_attentions as keyword argument.
...
class BartEncoderLayerNestedTransformer(NestedTransformerBaseLayer, nn.Module):
    def __init__(self, bart_layer, config):
        r"""
        A simple conversion of the `BartEncoderLayer` to its `NestedTransformer` implementation.
    ...
    def forward(
        ...
        **__,
        ...
        if output_attentions:
            raise ValueError(
                "output_attentions=True can not be supported with NestedTransformer."
            )
...
class MBartEncoderLayerNestedTransformer(NestedTransformerBaseLayer, nn.Module):
    def __init__(self, mbart_layer, config):
        r"""
        A simple conversion of the `MBartEncoderLayer` to its `NestedTransformer` implementation.
        Args:
    ...
    def forward(
        ...
        **__,
        ...
        if (
            ...
            hidden_states = F.dropout(
                self.act_fn_callable(
                    F.linear(hidden_states, self.linear1_weight, self.linear1_bias)
                ),
                p=self.activation_dropout,
                training=self.training,
            )
...
class DistilBertLayerNestedTransformer(NestedTransformerBaseLayer, nn.Module):
    def __init__(self, bert_layer, config):
        r"""
        A simple conversion of the Distill-BERTLayer to its `NestedTransformer` implementation.
    ...
    def forward(
        self, hidden_states, attn_mask, output_attentions: bool, head_mask=None, *_
    ):
        if output_attentions:
            raise ValueError(
                "output_attentions=True can not be supported with NestedTransformer."
        ...
        if (
            ...
            if attn_mask is not None:
                ...
                attn_mask = torch.reshape(
                    attn_mask, (attn_mask.shape[0], attn_mask.shape[-1])
                )
                seqlen = attn_mask.shape[1]
                lengths = torch.sum(~attn_mask, 1)
                if not all(le == seqlen for le in lengths):
                    hidden_states = torch._nested_tensor_from_mask(
...
class ViTLayerNestedTransformer(NestedTransformerBaseLayer, nn.Module):
    def __init__(self, vit_layer, config):
        r"""
        A simple conversion of the ViTLayer to its `NestedTransformer` implementation.
        ...
        self.embed_dim = int(
            vit_layer.attention.attention.attention_head_size * self.num_heads
        )
    ...
    def forward(self, hidden_states, output_attentions: bool, *_, **__):
        ...
        if (
            ...
            raise NotImplementedError(
                "Training and Autocast are not implemented for NestedTransformer + ViT. Please open an issue."
            )
        return (hidden_states,)


class ViltLayerNestedTransformer(NestedTransformerBaseLayer, nn.Module):
    def __init__(self, vilt_layer, config):
        r"""
        A simple conversion of the VilTLayer to its `NestedTransformer` implementation.
        ...
        self.embed_dim = int(
            vilt_layer.attention.attention.attention_head_size * self.num_heads
        )
    ...
    def forward(
        self, hidden_states, layer_head_mask, output_attentions: bool, *_, **__
    ):
        if output_attentions:
            raise ValueError(
                "output_attentions=True can not be supported with NestedTransformer."
        ...
        if (
            ...
            raise NotImplementedError(
                "Training and Autocast are not implemented for NestedTransformer + Vilt. Please open an issue."
            )
        return (hidden_states,)


class Wav2Vec2EncoderLayerNestedTransformer(NestedTransformerBaseLayer, nn.Module):
    def __init__(self, wav2vec2_layer, config):
        r"""
        A simple conversion of the Wav2Vec2EncoderLayer to its `NestedTransformer` implementation.
    ...
    def forward(self, hidden_states, attention_mask, output_attentions: bool, **__):
        ...
        if (
            ...
            raise NotImplementedError(
                "Training and Autocast are not implemented for NestedTransformer + Wav2Vec2. Please open an issue."
            )
        return (hidden_states,)


class FSMTEncoderLayerNestedTransformer(NestedTransformerBaseLayer, nn.Module):
    def __init__(self, fsmt_layer, config):
        r"""
        A simple conversion of the FSMT Encoder layer to its `NestedTransformer` implementation.
    ...
    def forward(
        ...
        **__,
        ...
        if (
            ...
            raise NotImplementedError(
                "Training and Autocast are not implemented for NestedTransformer + FSMT. Please open an issue."
            )

        return (hidden_states, attention_mask)


class ProphetNetEncoderLayerNestedTransformer(NestedTransformerBaseLayer, nn.Module):
    def __init__(self, prophetnet_layer, config):
        r"""
        A simple conversion of the ProphetNet Encoder layer to its `NestedTransformer` implementation.
        ...
        self.original_layers_mapping = {
            ...
            "in_proj_bias": [
                "self_attn.query_proj.bias",
                "self_attn.key_proj.bias",
                "self_attn.value_proj.bias",
            ],
            "out_proj_weight": "self_attn.out_proj.weight",
            "out_proj_bias": "self_attn.out_proj.bias",
    ...
    def forward(self, hidden_states, attention_mask, output_attentions: bool, *_, **__):
        ...
        if (
            ...
            raise ValueError(
                "Training and Autocast are not implemented for NestedTransformer + ProphetNet. Please open an issue."
            )

        return (hidden_states,)


class CLIPLayerNestedTransformer(NestedTransformerBaseLayer, nn.Module):
    def __init__(self, layer, config):
        r"""
        A simple conversion of the CLIPEncoderLayer to its `NestedTransformer` implementation.
        ...
        **__,
        ...
        if (
            ...
            if attention_mask is not None or causal_attention_mask is not None:
                raise ValueError(
                    "Please do not use attention masks when using `NestedTransformer` converted vision models"
                )
            ...
            raise NotImplementedError(
                "Training and Autocast are not implemented for NestedTransformer + CLIP. Please open an issue."
            )
