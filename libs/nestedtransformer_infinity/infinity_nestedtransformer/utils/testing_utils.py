...
def require_ort_rocm(test_case):
    ...
    return unittest.skipUnless(
        "ROCMExecutionProvider" == providers[0], "test requires ROCMExecutionProvider"
    )(test_case)
...
def require_hf_token(test_case):
    ...
    if hf_token is None:
        return unittest.skip(
            "test requires hf token as `HF_AUTH_TOKEN` environment variable"
        )(test_case)
    else:
        return test_case
...
def require_sigopt_token_and_project(test_case):
    ...
    if sigopt_api_token is None or has_sigopt_project is None:
        return unittest.skip(
            "test requires an environment variable `SIGOPT_API_TOKEN` and `SIGOPT_PROJECT`"
        )(test_case)
    else:
        return test_case
...
def is_ort_training_available():
    is_ort_train_available = (
        importlib.util.find_spec("onnxruntime.training") is not None
    )
    ...
    if importlib.util.find_spec("torch_ort") is not None:
        try:
            ...
            subprocess.run(
                [sys.executable, "-m", "torch_ort.configure"], shell=False, check=True
            )
...
def require_diffusers(test_case):
    return unittest.skipUnless(is_diffusers_available(), "test requires diffusers")(
        test_case
    )
...
def require_sentence_transformers(test_case):
    return unittest.skipUnless(
        is_sentence_transformers_available(), "test requires sentence-transformers"
    )(test_case)


def require_datasets(test_case):
    return unittest.skipUnless(is_datasets_available(), "test requires datasets")(
        test_case
    )
...
def grid_parameters(
    ...
    for params in itertools.product(*parameters.values()):
        ...
        if yield_dict is True:
            ...
            returned_list = (
                [test_name] + list(params) if add_test_name is True else list(params)
            )
            yield returned_list
