...
class Calibration:
    ...
    calibration_histogram_percentile: Optional[float] = field(
        default=None,
        metadata={
            "description": "The percentile used for the percentile calibration method."
        },
    )
...
class FrameworkArgs:
    opset: Optional[int] = field(
        default=11,
        metadata={"description": "ONNX opset version to export the model with."},
    )
    optimization_level: Optional[int] = field(
        default=0, metadata={"description": "ONNX optimization level."}
    )
...
class DatasetArgs:
    ...
    path: str = field(
        metadata={
            "description": "Path to the dataset, as in `datasets.load_dataset(path)`."
        }
    )
    eval_split: str = field(
        metadata={"description": 'Dataset split used for evaluation (e.g. "test").'}
    )
    ...
    ref_keys: List[str] = field(
        metadata={
            "description": "Dataset column used for references during evaluation."
        }
    )
    name: Optional[str] = field(
        default=None,
        metadata={
            "description": "Name of the dataset, as in `datasets.load_dataset(path, name)`."
        },
    )
    calibration_split: Optional[str] = field(
        default=None,
        metadata={"description": 'Dataset split used for calibration (e.g. "train").'},
    )
...
class BenchmarkTimeArgs:
    ...
    duration: Optional[int] = field(
        default=30,
        metadata={"description": "Duration in seconds of the time evaluation."},
    )
    warmup_runs: Optional[int] = field(
        default=10,
        metadata={
            "description": "Number of warmup calls to forward() before the time evaluation."
        },
    )
...
class _RunBase:
    model_name_or_path: str = field(
        metadata={
            "description": "Name of the model hosted on the Hub to use for the run."
        }
    )
    ...
    framework: Frameworks = field(
        metadata={"description": 'Name of the framework used (e.g. "onnxruntime").'}
    )
    framework_args: FrameworkArgs = field(
        metadata={"description": "Framework-specific arguments."}
    )
...
class _RunDefaults:
    ...
    per_channel: Optional[bool] = field(
        default=False,
        metadata={"description": "Whether to quantize per channel (default: `False`)."},
    )
    calibration: Optional[Calibration] = field(
        default=None,
        metadata={
            "description": "Calibration parameters, in case static quantization is used."
        },
    )
    task_args: Optional[TaskArgs] = field(
        default=None,
        metadata={"description": "Task-specific arguments (default: `None`)."},
    )
    ...
    max_eval_samples: Optional[int] = field(
        default=None,
        metadata={
            "description": "Maximum number of samples to use from the evaluation dataset for evaluation."
        },
    )
    time_benchmark_args: Optional[BenchmarkTimeArgs] = field(
        default=BenchmarkTimeArgs(),
        metadata={"description": "Parameters related to time benchmark."},
    )
...
class _RunConfigBase:
    ...
    metrics: List[str] = field(
        metadata={"description": "List of metrics to evaluate on."}
    )
...
class _RunConfigDefaults(_RunDefaults):
    batch_sizes: Optional[List[int]] = field(
        ...
        metadata={
            "description": "Batch sizes to include in the run to measure time metrics."
        },
    )
    input_lengths: Optional[List[int]] = field(
        default_factory=lambda: [128],
        metadata={
            "description": "Input lengths to include in the run to measure time metrics."
        },
    )
...
class Run(_RunDefaults, _RunBase):
    def __post_init__(self):
        ...
        assert (
            self.aware_training is False
        ), "Quantization-Aware Training not supported."
