...
class QuestionAnsweringProcessing(TaskProcessor):
    ...
    def dataset_processing_func(
        ...
        tokenized_inputs = self.preprocessor(
            text=example[
                data_keys["question"] if pad_on_right else data_keys["context"]
            ],
            text_pair=example[
                data_keys["context"] if pad_on_right else data_keys["question"]
            ],
            **self.defaults,
            **self.preprocessor_kwargs,
        )
    ...
    def try_to_guess_data_keys(
        ...
        for name in column_names:
            ...
            if context_key_name is None and (
                "sentence" in name or "context" in name or "answer" in name
            ):
