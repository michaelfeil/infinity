...
class ImageClassificationProcessing(TaskProcessor):
    ...
    def __init__(
        ...
        if size is None:
            raise ValueError(
                f"Could not retrieve the size information from {preprocessor}"
            )
        ...
        self.transforms = Compose(
            [
                ...
                ToTensor(),
                Normalize(
                    mean=self.preprocessor.image_mean, std=self.preprocessor.image_std
                ),
            ]
        )
    ...
    def dataset_processing_func(
        ...
        example["pixel_values"] = (
            self.transforms(image.convert("RGB")).to(torch.float32).numpy()
        )
        return example
