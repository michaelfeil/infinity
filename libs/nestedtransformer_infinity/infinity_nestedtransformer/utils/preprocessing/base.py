...
from infinity_nestedtransformer.utils.import_utils import requires_backends
...
class TaskProcessor(ABC):
    ...
    def __init__(
        ...
        ) = self.create_defaults_and_kwargs_from_preprocessor_kwargs(
            preprocessor_kwargs
        )
    ...
    def create_dataset_processing_func(
        ...
        return functools.partial(
            self.dataset_processing_func, data_keys=data_keys, ref_keys=ref_keys
        )
    ...
    def prepare_dataset(
        ...
        if isinstance(dataset, Dataset) and split is not None:
            raise ValueError(
                "A Dataset and a split name were provided, but splits are for DatasetDict."
            )
    ...
    def load_dataset(
        ...
        **load_dataset_kwargs,
        ...
        if isinstance(column_names, dict):
            column_names = list(
                set(itertools.chain.from_iterable(column_names.values()))
            )
        ...
        if ref_keys is None:
            ...
            logger.warning(
                f"As no ref keys were provided, tried to guess them: {ref_keys}."
            )
            ...
            if isinstance(dataset, DatasetDict):
                for split_name, split in dataset.items():
                    columns_to_remove = [
                        ...
                        if name not in necessary_columns
                    ]
    ...
    def load_default_dataset(
        ...
        **load_dataset_kwargs,
        ...
        if isinstance(self.DEFAULT_DATASET_ARGS, dict):
            ...
            common_keys = set(self.DEFAULT_DATASET_ARGS.keys()) & set(
            ...
            kwargs.update(
                {k: v for k, v in self.DEFAULT_DATASET_ARGS.items() if k != "path"}
            )
        else:
