...
class NormalizedConfig:
    ...
    def __init__(
        self, config: Union[PretrainedConfig, Dict], allow_new: bool = False, **kwargs
    ):
        self.config = config
    ...
    @classmethod
    def with_args(
        cls, allow_new: bool = False, **kwargs
    ) -> Callable[[PretrainedConfig], "NormalizedConfig"]:
...
class NormalizedTextAndVisionConfig(NormalizedTextConfig, NormalizedVisionConfig):
    ...
    def __getattr__(self, attr_name):
        if self.TEXT_CONFIG is not None and attr_name.upper() in dir(
            NormalizedTextConfig
        ):
            attr_name = f"{self.TEXT_CONFIG}.{attr_name}"
        elif self.VISION_CONFIG is not None and attr_name.upper() in dir(
...
class NormalizedEncoderDecoderConfig(NormalizedConfig):
    ...
    def __getattr__(self, attr_name):
        if (
            self.ENCODER_NORMALIZED_CONFIG_CLASS is not None
        ...
        if (
            self.DECODER_NORMALIZED_CONFIG_CLASS is not None
...
GPT2LikeNormalizedTextConfig = NormalizedTextConfig.with_args(
    num_attention_heads="n_head", hidden_size="n_embd"
)
...
class NormalizedConfigManager:
    ...
    _conf = {
        ...
        "distilbert": NormalizedTextConfig.with_args(
