...
def check_framework_is_available(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if (
            pt_asked_but_not_available or tf_asked_but_not_available
        ) and framework != "np":
            framework_name = "PyTorch" if framework == "pt" else "TensorFlow"
            raise RuntimeError(
                f"Requested the {framework_name} framework, but it does not seem installed."
            )
...
class DummyInputGenerator(ABC):
    ...
    def supports_input(self, input_name: str) -> bool:
        ...
        """
        return any(
            input_name.startswith(supported_input_name)
            for supported_input_name in self.SUPPORTED_INPUT_NAMES
        )
    ...
    def random_int_tensor(
        ...
        if framework == "pt":
            return torch.randint(
                low=min_value, high=max_value, size=shape, dtype=DTYPE_MAPPER.pt(dtype)
            )
        elif framework == "tf":
            return tf.random.uniform(
                shape, minval=min_value, maxval=max_value, dtype=DTYPE_MAPPER.tf(dtype)
            )
        else:
            return np.random.randint(
                min_value, high=max_value, size=shape, dtype=DTYPE_MAPPER.np(dtype)
            )
    ...
    def random_mask_tensor(
        ...
        if framework == "pt":
            ...
            mask_tensor = tf.concat(
                [
                    tf.ones(
                        (*shape[:-1], shape[-1] - mask_length),
                        dtype=DTYPE_MAPPER.tf(dtype),
                    ),
    ...
    def random_float_tensor(
        ...
        """
        if framework == "pt":
            tensor = torch.empty(shape, dtype=DTYPE_MAPPER.pt(dtype)).uniform_(
                min_value, max_value
            )
            return tensor
        elif framework == "tf":
    ...
    def pad_input_on_dim(
        ...
        if (desired_length is None and padding_length is None) or (
            ...
            raise ValueError(
                "You need to provide either `desired_length` or `padding_length`"
            )
        ...
        return cls.concat_inputs(
            [
                input_,
                cls.constant_tensor(
                    padding_shape, value=value, dtype=dtype, framework=framework
                ),
            ],
            dim=dim,
        )
...
class DummyXPathSeqInputGenerator(DummyTextInputGenerator):
    ...
    def generate(
        ...
        min_value = 0
        max_value = (
            self.tag_pad_id if input_name == "xpath_tags_seq" else self.subs_pad_id
        )
        shape = [self.batch_size, self.sequence_length, self.max_depth]
        return self.random_int_tensor(
            shape, max_value, min_value=min_value, framework=framework, dtype=int_dtype
        )
...
class DummySeq2SeqDecoderTextInputGenerator(DummyDecoderTextInputGenerator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(normalized_config, NormalizedEncoderDecoderConfig):
            self.hidden_size = (
                normalized_config.ENCODER_NORMALIZED_CONFIG_CLASS.hidden_size
            )
        else:
...
class DummySeq2SeqPastKeyValuesGenerator(DummyInputGenerator):
    ...
    def __init__(
        self,
        task: str,
        normalized_config: Union[
            NormalizedSeq2SeqConfig, NormalizedEncoderDecoderConfig
        ],
        batch_size: int = DEFAULT_DUMMY_SHAPES["batch_size"],
        sequence_length: int = DEFAULT_DUMMY_SHAPES["sequence_length"],
        ...
        **kwargs,
        ...
        if isinstance(normalized_config, NormalizedEncoderDecoderConfig):
            ...
            self.encoder_num_attention_heads = self.normalized_config.DECODER_NORMALIZED_CONFIG_CLASS.encoder_num_attention_heads
            self.decoder_num_attention_heads = self.normalized_config.DECODER_NORMALIZED_CONFIG_CLASS.decoder_num_attention_heads
            # Same, `encoder_hidden_size` and `decoder_hidden_size` are bad names.
            self.encoder_hidden_size = (
                self.normalized_config.DECODER_NORMALIZED_CONFIG_CLASS.hidden_size
            )
            self.decoder_hidden_size = (
                self.normalized_config.DECODER_NORMALIZED_CONFIG_CLASS.hidden_size
            )
            self.decoder_num_layers = (
                self.normalized_config.DECODER_NORMALIZED_CONFIG_CLASS.num_layers
            )
        else:
    ...
    def generate(
        ...
        if input_name == "past_key_values":
            ...
            return [
                (
                    self.random_float_tensor(
                        decoder_shape, framework=framework, dtype=float_dtype
                    ),
                    self.random_float_tensor(
                        decoder_shape, framework=framework, dtype=float_dtype
                    ),
                    self.random_float_tensor(
                        encoder_shape, framework=framework, dtype=float_dtype
                    ),
                    self.random_float_tensor(
                        encoder_shape, framework=framework, dtype=float_dtype
                    ),
...
class DummyLabelsGenerator(DummyInputGenerator):
    ...
    def generate(
        ...
        return self.random_int_tensor(
            shape, max_value=max_value, framework=framework, dtype=int_dtype
        )
...
class DummyPointsGenerator(DummyInputGenerator):
    ...
    def generate(
        ...
        if input_name == "input_points":
            ...
            return self.random_int_tensor(
                shape, min_value=0, max_value=1, framework=framework, dtype=int_dtype
            )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.output_channels = (
            ...
            else normalized_config.vision_config.output_channels
        )
...
class GPTBigCodeDummyPastKeyValuesGenerator(DummyPastKeyValuesGenerator):
    def generate(
        ...
        past_key_value_shape = (
            ...
            self.hidden_size
            // self.num_attention_heads
            * 2,  # GPT BigCode has a fused KV cache.
        )
        return [
            self.random_float_tensor(
                past_key_value_shape, framework=framework, dtype=float_dtype
            )
            for _ in range(self.num_layers)
        ]
...
class MultiQueryPastKeyValuesGenerator(DummyPastKeyValuesGenerator):
    ...
    def generate(
        ...
        return [
            (
                self.random_float_tensor(
                    past_shape, framework=framework, dtype=float_dtype
                ),
                self.random_float_tensor(
                    past_shape, framework=framework, dtype=float_dtype
                ),
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.num_kv_heads = self.num_kv_heads = (
            ...
            if (
                normalized_config.new_decoder_architecture
                or not normalized_config.multi_query
            )
            else 1
        )
    ...
    def generate(
        ...
        if input_name == "output_sequence":
            ...
            shape = [
                20,
                self.num_mel_bins,
            ]  # NOTE: the first axis length is arbitrary and dynamic
        else:
            raise ValueError(
                f"Unsupported input {input_name} for DummySpeechT5InputGenerator"
            )
...
class DummyVisionEncoderDecoderPastKeyValuesGenerator(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(
            normalized_config.DECODER_NORMALIZED_CONFIG_CLASS, NormalizedSeq2SeqConfig
        ):
...
class DummyCodegenDecoderTextInputGenerator(DummySeq2SeqDecoderTextInputGenerator):
    def __init__(
        ...
        **kwargs,
        ...
        self.num_codebooks = (
            normalized_config.DECODER_NORMALIZED_CONFIG_CLASS.num_codebooks
        )
...
class DummyEncodecInputGenerator(DummyInputGenerator):
    ...
    def generate(
        ...
        if input_name == "audio_codes":
            ...
            raise ValueError(
                f"Unsupported input {input_name} for DummyEncodecInputGenerator"
            )
...
class DummyIntGenerator(DummyInputGenerator):
    ...
    def generate(
        ...
        return self.random_int_tensor(
            shape=(1,), min_value=20, max_value=22, framework=framework, dtype=int_dtype
        )
...
class DummyTransformerTimestepInputGenerator(DummyTimestepInputGenerator):
    ...
    def generate(
        ...
        if input_name == "timestep":
            shape = [
                self.batch_size
            ]  # With transformer diffusers, timestep is a 1D tensor
            return self.random_float_tensor(
                shape, max_value=self.vocab_size, framework=framework, dtype=float_dtype
            )
...
class DummyTransformerTextInputGenerator(DummySeq2SeqDecoderTextInputGenerator):
    ...
    def generate(
        ...
        if input_name == "encoder_hidden_states":
            ...
            return self.random_float_tensor(
                [self.batch_size, self.normalized_config.projection_size],
                framework=framework,
                dtype=float_dtype,
            )
...
class DummyFluxTransformerVisionInputGenerator(DummyTransformerVisionInputGenerator):
    ...
    def generate(
        ...
        if input_name == "hidden_states":
            ...
            return self.random_int_tensor(
                shape, max_value=1, framework=framework, dtype=int_dtype
            )
