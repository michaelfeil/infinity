...
from .testing_utils import MODELS_DICT, NestedTransformersTestMixin
...
class NestedTransformersEncoderTest(NestedTransformersTestMixin):
    r"""
    Full testing suite of the `NestedTransformers` integration into Hugging Face
    `transformers` ecosystem. Check the docstring of each test to understand the
    purpose of each test. Basically we test:
    - if the conversion dictionary is consistent, ie if the converted model exists
    ...
    def test_raise_pos_emb(self):
        r"""
        Test if the conversion properly raises an error if the model has an activate function that is
        not supported by `NestedTransformer`. For now, only `Bert` family model support this test.
        """
        ...
        with self.assertRaises(ValueError):
            ...
            _ = NestedTransformer.transform(hf_model, keep_original_model=False)
    ...
    def test_inference_speed(self):
        ...
        bt_model = NestedTransformer.transform(hf_model, keep_original_model=True)
        ...
        input_ids, _, attention_mask = get_batch(
            BATCH_SIZE, SEQ_LEN, MAX_SEQ_LEN, STD_SEQ_LEN, VOCAB_SIZE
        )
        ...
        for _ in range(N_REPEAT):
            mean_hf_time += timeit.timeit(
                lambda: hf_model(input_ids, attention_mask=attention_mask), number=1
            )
            mean_bt_time += timeit.timeit(
                lambda: bt_model(input_ids, attention_mask=attention_mask), number=1
            )
        ...
        self.assertLess(
            mean_bt_time,
            mean_hf_time,
            "The converted model is slower than the original model.",
        )
        gc.collect()
    ...
    def test_pipeline_on_gpu(self):
        ...
        model_name = "distilbert-base-uncased"
        unmasker = pipeline(
            "fill-mask", model_name, accelerator="bettertransformer", device="cuda:0"
        )
    ...
    @require_torch_gpu
    @require_accelerate
    def check_accelerate_compatibility_cpu_gpu(
        self, keep_original_model=True, max_memory=None
    ):
        r"""
        This tests if a model loaded with `accelerate` will be successfully converted
        into its NestedTransformers format.
        ...
        self.assertEqual(
            bt_model.encoder.layer[0].in_proj_weight.device, torch.device("cuda:0")
        )
        # Weights that are offloaded on the CPU are offloaded on the `meta` device
        if "cpu" in set(max_memory):
            self.assertEqual(
                bt_model.encoder.layer[-1].in_proj_weight.device, torch.device("meta")
            )
        ...
        self.assertTrue(
            torch.allclose(output_bt[0][0, :3], output_hf[0][0, :3], atol=1e-3)
        )

        # Check that the padding has been taken into account correctly - this checks also if the hooks
        # have been correctly set.
        self.assertTrue(
            torch.allclose(output_bt[0][1, 3:], torch.zeros_like(output_bt[0][1, 3:]))
        )
        gc.collect()
    ...
    def test_accelerate_compatibility_cpu_gpu(self):
        ...
        self.check_accelerate_compatibility_cpu_gpu(
            keep_original_model=True, max_memory=max_memory
        )
    ...
    def test_logits(self, test_name: str, model_type: str, batch_size: int):
        ...
        self._test_logits(
            model_id=model_id, model_type=model_type, batch_size=batch_size
        )
    ...
    def test_invert_modules(
        self, test_name: str, model_type: str, keep_original_model=False
    ):
        model_id = MODELS_DICT[model_type]
