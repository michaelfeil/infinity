...
from infinity_nestedtransformer import NestedTransformer
from infinity_nestedtransformer.utils.testing_utils import (
    flatten_dict,
    require_torch_gpu,
)
...
MODELS_DICT = {
    ...
    # NOTE: gpt_bigcode directly supports SDPA in Transformers.
    ...
    # NOTE: llama directly supports SDPA in Transformers.
    ...
    # NOTE: whisper directly supports SDPA in Transformers.
...
class NestedTransformersTestMixin(unittest.TestCase):
    r"""
    `NestedTransformersTestMixin` to wrap necessary functions for testing `NestedTransformer`
    integration. This includes the following tests:
        - `test_logits`: This tests if the converted model produces the same logits
        than the original model.
        - `test_raise_on_save`: Test if the conversion properly raises an error if someone tries to save the model using `save_pretrained`.
    """
    ...
    def _test_fp16_inference(
        ...
        **preprocessor_kwargs,
        ...
        inputs = self.prepare_inputs_for_class(
            model_id=model_id, model_type=model_type, **preprocessor_kwargs
        ).to(0)
        ...
        if not use_to_operator:
            ...
            converted_model = NestedTransformer.transform(
                hf_random_model, keep_original_model=True
            )
        else:
            hf_random_model = automodel_class.from_pretrained(
        ...
        with torch.no_grad():
            ...
            output_hf = hf_random_model.generate(
                **inputs, min_length=length, max_length=length
            )
    ...
    def _test_logits_backward(
        self, model_id: str, model_type: str, **preprocessor_kwargs
    ):
        inputs = self.prepare_inputs_for_class(
            model_id=model_id, model_type=model_type, **preprocessor_kwargs
        )

        hf_random_model = AutoModel.from_pretrained(
            model_id, attn_implementation="eager"
        ).eval()
        ...
        converted_model = NestedTransformer.transform(converted_model)
    ...
    def _test_logits(self, model_id: str, model_type: str, **preprocessor_kwargs):
        ...
        with torch.no_grad():
            ...
            if "quick_gelu" in flatten_dict(random_config.to_dict()).values():
                # Since `quick_gelu` is a rather slightly modified version of `GeLU` we expect a discrepancy.
                tol = 3e-1
    ...
    def assert_equal(self, tensor1, tensor2, atol: float, model_name: str):
        self.assertTrue(
            ...
            f"The NestedTransformer converted model does not produce the same logits as the original model. Failed for the model {model_name}."
    ...
    def _test_train_decoder(self, model_id: str, model_type: str, **kwargs):
        ...
        """
        inputs = self.prepare_inputs_for_class(
            model_id=model_id, model_type=model_type, **kwargs
        )
        ...
        bt_model = NestedTransformer.transform(
            hf_random_model, keep_original_model=True
        )
        bt_model.train()
        ...
        for name, param in bt_model.named_parameters():
            ...
            if (
                hf_random_model.config.model_type == "pegasus"
                and "embed_positions.weight" in name
            ):
                continue
    ...
    def _test_invert_modules(self, model_id, keep_original_model=False):
        ...
        bt_model = NestedTransformer.transform(
            hf_model, keep_original_model=keep_original_model
        )
        bt_model = NestedTransformer.reverse(bt_model)
        ...
        for hf_module, bt_module in zip(hf_modules, bt_modules):
            ...
            self.assertEqual(
                inspect.signature(hf_module.forward),
                inspect.signature(bt_module.forward),
            )
            self.assertEqual(
                inspect.signature(hf_module.__init__),
                inspect.signature(bt_module.__init__),
            )

            self.assertEqual(
                inspect.getsource(hf_module.forward),
                inspect.getsource(bt_module.forward),
            )
            self.assertEqual(
                inspect.getsource(hf_module.__init__),
                inspect.getsource(bt_module.__init__),
            )
    ...
    def _test_invert_model_logits(
        ...
        **preprocessor_kwargs,
        ...
        """
        inputs = self.prepare_inputs_for_class(
            model_id, model_type=model_type, **preprocessor_kwargs
        )
        ...
        for i in range(len(output_bt)):
            if isinstance(output_bt[i], torch.Tensor):
                ...
                flattened_output_bt = [
                    out for j in range(len(output_bt[i])) for out in output_bt[i][j]
                ]
                flattened_output_hf = [
                    out for j in range(len(output_hf[i])) for out in output_hf[i][j]
                ]
                for j in range(len(flattened_output_bt)):
                    if isinstance(flattened_output_bt[j], torch.Tensor):
                        self.assertTrue(
                            torch.allclose(
                                flattened_output_bt[j],
                                flattened_output_hf[j],
                                atol=1e-4,
                            ),
                        ...
                        for k in range(len(flattened_output_bt[j])):
                            self.assertTrue(
                                torch.allclose(
                                    flattened_output_bt[j][k],
                                    flattened_output_hf[j][k],
                                    atol=1e-4,
                                ),
...
def get_batch(
    batch_size, avg_seqlen, max_sequence_length, seqlen_stdev, vocab_size, pad_idx=0
):
    r"""
    ...
    # need at least a sequence length of 1 for NestedTransformer to work
