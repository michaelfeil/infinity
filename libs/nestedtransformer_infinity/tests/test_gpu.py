...
from infinity_nestedtransformer.utils import logging
...
def benchmark(
    model_name: str, num_batches: int, batch_size: int, max_seqlen: int, is_half: bool
):
    hf_model = AutoModel.from_pretrained(
        model_name,
        torch_dtype=torch.float16 if is_half else None,
    ...
    input_ids = (
        torch.randint(vocab_size - 1, (batch_size, max_seqlen), dtype=torch.int64) + 1
    )
    ...
    total_hf_time = timing_cuda(
        hf_model, num_batches, input_ids, masks, decoder_input_ids
    )
    total_bt_time = timing_cuda(
        bt_model, num_batches, input_ids, masks, decoder_input_ids
    )
...
class TestSpeedup(unittest.TestCase):
    ...
    REPRESENTATIVE_MODELS = [
        "bert-base-uncased",
        # "albert-base-v2",  # TODO: AlbertLayerNestedTransformer seem to nest/unnest tensors all the time
        "facebook/bart-base",
    ...
    @unittest.skipIf(
        int(os.environ.get("TEST_LEVEL", 0)) < 1, reason="disabled by default"
    )
    def test_base_speedup(
        ...
        """
        Test to validate the NestedTransformer base speedup on GPU.
        ...
        self.assertTrue(
            speedup > 0.85,
            msg=f"The NestedTransformer base speedup for {test_name} is {speedup}",
        )

        if speedup >= 0.85 and speedup < 1:
            logger.warning(
                f"The NestedTransformer base speedup for {test_name} is {speedup}"
            )
        if speedup >= 1:
            logger.info(
                f"The NestedTransformer base speedup for {test_name} is {speedup}"
