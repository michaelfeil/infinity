...
from infinity_nestedtransformer import NestedTransformer, NestedTransformerManager
from optimum.pipelines import pipeline
from infinity_nestedtransformer.utils.testing_utils import grid_parameters


class NestedTransformerIntegrationTests(unittest.TestCase):
    def test_raise_error_on_double_transform_call(self):
        ...
        with self.assertRaises(Exception) as cm:
            bt_model = NestedTransformer.transform(model)
            bt_model = NestedTransformer.transform(bt_model)
        self.assertTrue(
            "was called on a model already using Better Transformer"
            in str(cm.exception)
        )

    @patch("optimum.utils.import_utils.is_onnxruntime_available")
    def test_direct_pipeline_initialization_without_onnx_installed(
        self, mock_onnxruntime_availability
    ):
    ...
    def test_raise_on_save(self, model_type: str):
        ...
        model_ids = (
            ...
            if isinstance(MODELS_DICT[model_type], tuple)
            else (MODELS_DICT[model_type],)
        )
        for model_id in model_ids:
            with self.assertRaises(
                ...
                bt_model = NestedTransformer.transform(
                    hf_model, keep_original_model=False
                )
    ...
    def test_conversion(self, model_type: str):
        r"""
        This tests if the conversion of a slow model to its NestedTransformer version using fastpath
        has been successful.
        """
        model_ids = (
            MODELS_DICT[model_type]
        ...
        for model_id in model_ids:
            ...
            converted_model = NestedTransformer.transform(hf_random_model)
    ...
    @parameterized.expand(
        grid_parameters(
            {"model_type": MODELS_DICT.keys(), "keep_original_model": [True, False]}
        )
    )
    def test_raise_save_pretrained_error(
        self, test_name: str, model_type: str, keep_original_model: bool
    ):
        r"""
        ...
        for model_id in model_ids:
            ...
            model = NestedTransformer.reverse(bt_model)
            with tempfile.TemporaryDirectory() as tmpdirname:
                model.save_pretrained(tmpdirname)

    @parameterized.expand(NestedTransformerManager.MODEL_MAPPING.keys())
    def test_raise_activation_fun(self, model_type: str):
        ...
        that is not supported by `NestedTransformer`. Here we need to loop over the config files
        """
        if NestedTransformerManager.requires_strict_validation(model_type) is False:
            self.skipTest(
                "The architecture does not require a specific activation function"
            )

        if model_type in ["wav2vec2", "hubert", "bark"]:
            self.skipTest(
                "These architectures do not support deepcopy (raise unrelated error)"
            )

        layer_classes = NestedTransformerManager.MODEL_MAPPING[model_type].keys()
        for layer_class in layer_classes:
            ...
            hf_random_model = AutoModel.from_config(
                hf_random_config, attn_implementation="eager"
            ).eval()

            with self.assertRaises(ValueError) as cm:
                _ = NestedTransformer.transform(
    ...
    def test_dict_class_consistency(self):
        """
        A test to check NestedTransformerManager.MODEL_MAPPING has good names.
        """
        for _, item in NestedTransformerManager.MODEL_MAPPING.items():
            self.assertTrue(
                any(
