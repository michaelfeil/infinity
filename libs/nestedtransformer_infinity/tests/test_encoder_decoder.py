...
class NestedTransformersEncoderDecoderTest(
    NestedTransformersTestMixin, unittest.TestCase
):
    r"""
    ...
    def test_fp16_inference(
        self, test_name: str, model_type: str, use_to_operator: bool
    ):
        # TODO: fix in transformers
        if model_type == "fsmt":
            self.skipTest(
                "fsmt is broken is transformers when loaded through torch_dtype=torch.float16"
            )
