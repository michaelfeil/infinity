...
class TestsWhisper(unittest.TestCase):
    def test_error_message(self):
        ...
        with self.assertRaises(ValueError) as cm:
            model = NestedTransformer.transform(model)

        self.assertTrue(
            "Transformers now supports natively NestedTransformer optimizations"
            in str(cm.exception)
        )


class NestedTransformersBarkTest(NestedTransformersTestMixin, unittest.TestCase):
    r"""
    Testing suite for Bark - tests all the tests defined in `NestedTransformersTestMixin`
    Since `Bark` is a text-to-speech model, it is preferable
        ...
        **preprocessor_kwargs,
        ...
        if not use_to_operator:
            ...
            converted_model = NestedTransformer.transform(
                hf_random_model, keep_original_model=False
            )
        ...
        with torch.inference_mode():
            ...
            output_hf = hf_random_model.generate(
                **inputs,
                ...
                do_sample=False,
                semantic_max_new_tokens=length,
            )
    ...
    def test_fp16_inference(
        self, test_name: str, model_type: str, use_to_operator: bool, batch_size: int
    ):
    ...
    @parameterized.expand(
        grid_parameters({"model_type": SUPPORTED_ARCH, "batch_size": [1, 2]})
    )
...
class NestedTransformersAudioTest(NestedTransformersTestMixin, unittest.TestCase):
    r"""
    Testing suite for Audio models - tests all the tests defined in `NestedTransformersTestMixin`
    """
    ...
    def test_logits(self, model_type: str):
        ...
        for model_id in model_ids:
            ...
            with torch.no_grad():
                ...
                self.assertTrue(
                    torch.allclose(
                        bt_hidden_states[0][-3:],
                        torch.zeros_like(bt_hidden_states[0][-3:]),
                    ),
                    "The NestedTransformers converted model does not give null hidden states on padded tokens",
                )

                self.assertTrue(
                    torch.allclose(
                        hf_hidden_states[-1], bt_hidden_states[-1], atol=tol
                    ),
                    "The NestedTransformers Converted model does not produce the same logits as the original model. Failed for the model {}".format(
